...
# 【1】方案 A：强行保持 100% 在线（耗电警告 ⚠️）
#   - 逻辑：持有内核级 wake_lock 唤醒锁。
#   - 后果：手机 CPU 将永远无法进入深度休眠。
#   - 体验：主力机会出现极其严重的“待机耗电”（一晚上待机可能掉电 20%-30% 甚至更多），
#           并且由于 CPU 始终在底层活跃，手机放口袋里可能会温热。适合 24 小时插电的备用机。

# 【2】方案 B：顺应系统休眠（推荐主力机使用 ✅）
#   - 逻辑：不持有唤醒锁，顺应安卓电源管理机制。
#   - 后果：当手机息屏并在口袋里放置一段时间后，安卓系统会进入 Doze（打盹）模式并冻结
#           后台底层网络活动。此时，哪吒面板上该设备会显示离线。
#   - 体验：对手机电池续航几乎零影响。当你点亮屏幕看微信、回消息时，系统被唤醒，
